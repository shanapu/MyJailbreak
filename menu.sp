...
public Action:EventDays(client, args)
{
	...
	} else PrintToChat(client, "%t %t", "menu_tag", "menu_notwarden" );
}
...
public JBMenuHandler(Handle:menu, MenuAction:action, client, itemNum) 
{
	if ( action == MenuAction_Select ) 
	{
		...
		else if ( strcmp(info,"ffa1") == 0 ) 
		{
			...
			PrintToChatAll("%t %t", "menu_tag", "menu_ffison" );
			JbMenu(client,0);
		}
		else if ( strcmp(info,"ffa2") == 0 ) 
		{
			SetCvar("mp_teammates_are_enemies", 0);
			PrintToChatAll("%t %t", "menu_tag", "menu_ffisoff" );
			JbMenu(client,0);
		}
		else if ( strcmp(info,"kill") == 0 ) 
		{
			...
			if(clientV > 0)
			{
				...
				PrintToChatAll("%t %t", "menu_tag", "menu_israndomdead, clientV);  //%N

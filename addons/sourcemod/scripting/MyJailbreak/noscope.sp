...
public void OnPluginStart()
{
	...
	gc_bAllowLR = AutoExecConfig_CreateConVar("sm_noscope_allow_lr", "1" , "0 - disabled, 1 - enable, LR on last round", _, true, 0.0, true, 1.0);
...
public Action SetNoScope(int client,int args)
{
	if (gc_bPlugin.BoolValue)
	{
		...
		{
			StartNextRound();
			if(MyJBLogging(true)) LogToFileEx(g_sEventsLogFile, "Event noscope was started by groupvoting");
		}

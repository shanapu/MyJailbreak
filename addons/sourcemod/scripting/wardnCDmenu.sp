...
public void OnPluginStart() 
{
	...
	RegConsoleCmd("sm_countdown", SetCountDownMenu);
...
public Action:SetCountDownMenu(client,args)
{
	if(gc_bCountDown.BoolValue)
	{
		if (warden_iswarden(client))
		{
			char cdmenu1[255], cdmenu2[255], cdmenu3[255], cdmenu4[255];
			
			Handle CountDownMenu = CreateMenu(Menu_Options);
			
			Format(cdmenu1, sizeof(cdmenu1), "%T\n ", "warden_cdmenu_Title", LANG_SERVER);
			SetMenuTitle(CountDownMenu, cdmenu1);
			Format(cdmenu2, sizeof(cdmenu2), "%T", "warden_cdmenu_start", LANG_SERVER);
			AddMenuItem(CountDownMenu, "start", cdmenu2);
			Format(cdmenu3, sizeof(cdmenu3), "%T", "warden_cdmenu_stop", LANG_SERVER);
			AddMenuItem(CountDownMenu, "stop", cdmenu3);
			Format(cdmenu4, sizeof(cdmenu4), "%T", "warden_cdmenu_startstop", LANG_SERVER);
			AddMenuItem(CountDownMenu, "startstop", cdmenu4);
			SetMenuExitButton(CountDownMenu, true);
		}else PrintToChatAll("kein warden");
	}else PrintToChatAll("aus");
}
...
public Menu_Options(Handle:menu, MenuAction:action, client, param2)
{
	if (action == MenuAction_Select)
	{
		...
		Format(g_sCountDownSelect, sizeof(g_sCountDownSelect), info);
		...
		{
			FakeClientCommand(client, "sm_stopcountdown");
		}
		else if (StrEqual(info, "startstop"))
		{
			FakeClientCommand(client, "sm_startstopcountdown");
		}
	}
}
